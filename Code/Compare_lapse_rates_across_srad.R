pertab <- data.frame(matrix(ncol=3,nrow=4))
names(pertab) <- c('season','dmean','dsd')
pertab$season <- c(sets$season)

da <- data.frame()
dl <- data.frame()

for (ss in 1:4){ # for each season:
  sm <- season_means %>% filter(season==unique(seasons$season[ss]))
  thres <- median(sm$srad)
  
  out <- sm %>% filter(srad >thres) %>% group_by(year,season) %>%
    summarise(lapse_high=summary(lm(smtemp~elev))$coefficients[2]*1000)
  
  out2 <- sm %>% filter(srad<thres) %>% group_by(year,season) %>%
  ...
  da <- rbind(da, highlow %>% group_by(season) %>%
  ...
  dd <- sm %>% filter(srad>thres) 
  sts <- length(unique(dd$station_id))
  for (ii in 1:1000){
    s1 <- unique(sm$station_id) %>% sample(sts)
    set1 <- sm %>% filter(station_id %in% s1) %>% group_by(season,year) %>%
    ...
    set2 <- sm %>% filter(!station_id %in% s1) %>% group_by(season,year) %>%
      mutate(lapse=summary(lm(smtemp~elev))$coefficients[2]*1000) %>% group_by(season) %>%
  ...
  dl <- rbind(dl, diflapse %>% gather('metric','value',dmean:dsd))
}
  ...
  ggplot(dl, aes(season,(value))) + geom_boxplot() + facet_wrap(~metric, ncol=1, scales="free",labeller=plot_labeller) +
    ...
    labs(y='C/km',x='Season',title=paste0('Comparing lapse rates across solar radiation classes'))
